...
-(UIButton *)createBtnWithImage:(NSString *)imageName companyName:(NSString *)name terminal:(NSString *)myTerminal phone:(NSString *)myPhone index:(NSInteger)index{
    ...
    //btn选中后的颜色
    UIImage * backImage = [UIImage imageNamed:@"btn_ distribution_click.png"];
    [myButton setImage:backImage forState:UIControlStateHighlighted];
...
-(void)detailBtnClick:(UIButton *)btn {
    NSMutableString * phoneNumber = [[NSMutableString alloc]initWithCapacity:0];
    for (id obj in btn.subviews) {
        UIView * phoneview = (UIView *)obj;
        if (phoneview.tag == 999) {
            UILabel * phoneLabel = (UILabel *)obj;
            [phoneNumber setString:phoneLabel.text];
        }
    }
    ...
    if([deviceType  isEqualToString:@"iPod touch"]||[deviceType  isEqualToString:@"iPad"]||[deviceType  isEqualToString:@"iPhone Simulator"]){
        ...
        NSString * temp = [NSString stringWithFormat:@"tel:%@",phoneNumber];
