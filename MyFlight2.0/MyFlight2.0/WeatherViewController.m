...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (myArray) {
        ...
        if (indexPath.row == 0) {
            ...
            NSMutableString * bigPicName = [[myArray objectAtIndex:0]objectForKey:@"weather"];
            for (int i = 0; i < [tempPicNameArray count]; i++) {
                if ([bigPicName hasSuffix:[tempPicNameArray objectAtIndex:i]]) {
            ...
            NSMutableString * littlePicName = [[myArray objectAtIndex:indexPath.row]objectForKey:@"weather"];
            NSMutableString * selectPicName = [[NSMutableString alloc]initWithCapacity:0];
            for (int i = 0; i < [tempPicNameArray count]; i++) {
...
//            [imageView setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@.png", littlePicName]]];
            NSString * str = [NSString stringWithFormat:@"%@.png",selectPicName];
            UIImage * image = [UIImage imageNamed:str];
            NSLog(@"image name :%@",str);
            UIImageView * imageView = [[UIImageView alloc]initWithImage:image];
            imageView.frame = CGRectMake(69, 10, 22, 22);
